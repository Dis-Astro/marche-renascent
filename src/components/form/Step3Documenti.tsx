  ...
  if (tipo === "proprietario") {
    return (
      <div className="space-y-4">
        ...
        <CheckboxGroup label="Hai già un progetto pronto?" value={form.progetto} options={SI_NO_NONSO} onChange={(v) => update("progetto", v)} />

        <CheckboxGroup label="Hai già ottenuto il buono contributo per avviare i lavori?" value={form.buono_contributo} options={SI_NO_NONSO} onChange={(v) => update("buono_contributo", v)} />
        ...
        <div>
          <label className="text-xs font-semibold text-foreground block mb-1">
            Aggiungi ulteriori informazioni che ritieni utile farci sapere <span className="text-[10px] text-primary/60 ml-1">Facoltativo</span>
          </label>
          <textarea
            value={form.note || ""}
